...
## 2026-06-20 building level cap and ascension feedback
- Extended generated building data from Lv.5 max to Lv.30 max, including regenerated `Assets/Data/Tables/buildings.json` and `web-game/runtime-data.json`.
- Added runtime building stage tiers at Lv.1, 5, 10, 15, 20, 25, and 30 with increasing building power multipliers.
- Added legacy save repair so existing saves with `maxLevel: 5` are upgraded to the current Lv.30 cap when loaded.
- Reworked post-Lv.5 production scaling so buildings continue growing without the old Lv.5 dead end.
- Connected building stages to actual systems: building power now scales by stage, party max HP gains part of building power, hero/kingdom buildings reduce hero training cost, and crafting/research buildings raise chest item levels.
- Added village card stage badges, five-step milestone tracks, stronger ascension result panels, card glow, reward burst, and upgrade burst when a building crosses a 5-level milestone.
- Updated service worker cache to `lost-kingdom-runtime-v29`.
- Additional optional art backlog: if more visual polish is needed, create building-state images for each major era (`lv01_ruins`, `lv05_village`, `lv10_city`, `lv15_kingdom`, `lv20_empire`, `lv25_golden_empire`, `lv30_capital`) for the 12 building types. Current implementation does not require these images; it uses overlays and existing building art.
- Verification passed: `npm run check` completed with 37/37 tests passing plus data, prompt, asset, sound, preview, runtime, and release validation.
- Browser verification passed with a legacy save containing `building_farm` at `level: 9` and `maxLevel: 5`: load repaired the cap to 30, the button showed `승급 강화`, upgrade moved the farm to Lv.10 `도시 단계`, and screenshot saved to `output/playwright/building-ascension-v29.png`.

## 2026-06-20 grouped building era growth
- Replaced independent post-Lv.5 building growth with grouped era gates: all buildings must reach the current cap before the next 5-level range unlocks.
- New games start with building cap Lv.5, then unlock Lv.6-10, 11-15, 16-20, 21-25, and 26-30 through whole-village era ascension.
- Added runtime APIs for `buildingEraInfo`, `canAdvanceBuildingEra`, `advanceBuildingEra`, and `syncBuildingLevelCaps`, including legacy save inference for existing higher-level buildings.
- Added a village era gate card that shows whole-building progress, a strong ready state, a one-tap era ascension action, and a visible newly unlocked range result.
- Updated building cards so capped buildings show current range complete and route the player to the era card instead of pretending an individual building can keep leveling.
- Added `docs/BUILDING_ERA_IMAGE_PROMPTS.md` covering 12 buildings across 6 visual states (`lv05_village` through `lv30_capital`) for future image generation.
- Updated service worker cache to `lost-kingdom-runtime-v30`.
- Verification passed: targeted building era prompt/runtime tests completed 15/15.
- Browser verification passed at `http://127.0.0.1:5176/web-game/?v=building-era-v30-final`: all 12 buildings at Lv.5 opened the ready era card, ascension changed every building cap to Lv.10, 12 ascended result panels appeared, and the screenshot was saved to `output/playwright/building-era-gate-v30.png`.
